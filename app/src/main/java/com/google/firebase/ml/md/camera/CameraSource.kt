...
class CameraSource(private val graphicOverlay: GraphicOverlay) {
    ...
    init {
        this.context = graphicOverlay.context
    }
    ...
    private fun setPreviewAndPictureSize(camera: Camera, parameters: Parameters) {
        ...
        var sizePair = PreferenceUtils.getUserSpecifiedPreviewSize(context)
        if (sizePair == null) {
            ...
            if (Utils.isPortraitMode(graphicOverlay.context)) {
                displayAspectRatioInLandscape = graphicOverlay.height.toFloat() / graphicOverlay.width
            } else {
                displayAspectRatioInLandscape = graphicOverlay.width.toFloat() / graphicOverlay.height
            }
    ...
    private fun setRotation(camera: Camera, parameters: Camera.Parameters) {
        ...
        when (deviceRotation) {
            ...
            else -> Log.e(TAG, "Bad device rotation value: $deviceRotation")
        }
    ...
    companion object {
        ...
        private val MIN_CAMERA_PREVIEW_WIDTH = 400
        private val MAX_CAMERA_PREVIEW_WIDTH = 1300
        private val DEFAULT_REQUESTED_CAMERA_PREVIEW_WIDTH = 640
        private val DEFAULT_REQUESTED_CAMERA_PREVIEW_HEIGHT = 360
        private val REQUESTED_CAMERA_FPS = 30.0f
        ...
        private fun selectSizePair(camera: Camera, displayAspectRatioInLandscape: Float): CameraSizePair? {
            ...
            var minAspectRatioDiff = java.lang.Float.MAX_VALUE
            ...
            if (selectedPair == null) {
                ...
                for (sizePair in validPreviewSizes) {
                    ...
                    val diff = Math.abs(size.width - CameraSource.DEFAULT_REQUESTED_CAMERA_PREVIEW_WIDTH) + Math.abs(size.height - CameraSource.DEFAULT_REQUESTED_CAMERA_PREVIEW_HEIGHT)
                    if (diff < minDiff) {
                        selectedPair = sizePair
                        minDiff = diff
                    }
